...
export function AppShell({ children }: React.PropsWithChildren) {
  ...
  const handleLogout = React.useCallback(() => {
    startLogoutTransition(async () => {
      try {
        await logout();
        toast.success("Đã đăng xuất khỏi WaveStream.");
      } catch (error) {
        toast.error(error instanceof Error ? error.message : tCommon.somethingWentWrong);
      }
    });
  }, [logout, tCommon.somethingWentWrong]);
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-background text-foreground">
      {/* Top Navigation Bar */}
      <header className="z-40 flex h-[60px] shrink-0 items-center border-b border-border bg-card px-4 gap-3">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2 shrink-0 mr-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary">
            <Music4 className="h-4 w-4 text-white" />
          </div>
          <span className="hidden font-bold text-foreground sm:block tracking-tight">WaveStream</span>
        </Link>
          ...
          {user ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="min-w-52">
                ...
                <DropdownMenuItem asChild>
                  <Link href="/about">
                    <span className="mr-2 h-4 w-4 text-base leading-none">ℹ</span>
                    {tCommon.about}
                  </Link>
                </DropdownMenuItem>
